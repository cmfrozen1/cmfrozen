        ...
        const $ = {
            ...
            triggerStatusBox: document.getElementById('trigger-status-box')
        };

        // ========== UTILITY FUNCTIONS ==========
        function updateRoleFlags() {
            state.isAdmin = state.role === 'admin';
            state.isManager = state.isAdmin || state.role === 'manager' || state.role === 'operator';
            console.log('Role updated:', { role: state.role, isAdmin: state.isAdmin, isManager: state.isManager });
        }

        function applyRoleBasedMenu() {
            const isOperator = state.isManager && !state.isAdmin;
            document.querySelectorAll('.admin-only-pending, .admin-only-users, .admin-only-settings').forEach(el => {
                el.style.display = state.isAdmin ? '' : 'none';
            });

            if (isOperator) {
                document.querySelectorAll('.admin-tab-btn').forEach(btn => {
                    btn.classList.toggle('active', btn.dataset.adminTab === 'bookings');
                });
        ...
        function showToast(msg, type = 'success') {
            ...
            const duration = msg && msg.length > 40 ? 7000 : 3000;
            setTimeout(() => {
        ...
        async function setupSystemTriggers() {
            if (!state.isAdmin) {
            ...
            try {
                const result = await callGAS('admin/setup-triggers', {
                    lineUserId: state.user.lineUserId
                });

                if (result.success) {
                    showToast('✅ ตั้งค่า System Triggers เรียบร้อย', 'success');
                    console.log('📊 Trigger setup result:', result.data);

                    if (result.data && result.data.triggers) {
                        let msg = '📋 รายการ Triggers ที่สร้าง:\n';
                        result.data.triggers.forEach(t => {
                            msg += `• ${t.name} (${t.schedule})\n`;
                        });
                    ...
                    showToast(result.message || 'ตั้งค่าไม่สำเร็จ', 'error');
                }
            } catch (error) {
        ...
        async function checkTriggerStatus() {
            ...
            try {
                const result = await callGAS('admin/trigger-status', {
                    lineUserId: state.user.lineUserId
                });
                ...
                if (result.success) {
                    ...
                    if (data.status && data.status.setupAt) {
                        html += `<div class="text-[#06c755] mb-2">✅ ตั้งค่าเมื่อ: ${formatDateTime(data.status.setupAt)}</div>`;
                    } else {
                        html += `<div class="text-[#f59e0b] mb-2">⚠️ ยังไม่มีการตั้งค่า Triggers</div>`;
                    }

                    if (data.status && data.status.triggers) {
                        html += '<div class="text-gray-400 text-xs mb-2">📌 รายการ Triggers ที่ตั้งค่า:</div>';
                        data.status.triggers.forEach(t => {
                            html += `<div class="status-item"><span class="dot active"></span> ${t.name} <span class="text-gray-500 text-xs">(${t.schedule})</span></div>`;
                        });
                    }

                    if (data.activeTriggers && data.activeTriggers.length > 0) {
                        html += `<div class="text-gray-400 text-xs mt-2">🔹 Active Triggers: ${data.activeTriggers.length} ตัว</div>`;
                    } else {
                        html += `<div class="text-[#ef4444] text-xs mt-2">❌ ไม่มี Active Triggers</div>`;
                    }

                    html += '</div>';

                    $.triggerStatusBox.innerHTML = html;
                    $.triggerStatusBox.classList.remove('hidden');
                    $.triggerStatusBox.classList.add('success');
                }
            } catch (error) {
                console.error('Check trigger status error:', error);
            }
        }

        async function runTriggerNow(triggerName) {
            if (!state.isAdmin) {
                showToast('เฉพาะผู้ดูแลระบบเท่านั้น', 'error');
                return;
            }
            ...
            try {
                let path = '';
                let data = { lineUserId: state.user.lineUserId };

                switch (triggerName) {
                    case 'sendMeetingReminders':
                        path = 'admin/run-reminder';
                        break;
                    case 'autoCancelOverdueBookings':
                        path = 'admin/run-auto-cancel';
                        data.silentMode = 'true';
                        break;
                    default:
                        showToast('ไม่พบ Trigger ที่ระบุ', 'error');
                        setButtonLoading(btn, false);
                        return;
                }
                ...
                if (result.success) {
                    showToast(`✅ รัน ${triggerName} สำเร็จ`, 'success');
                    console.log('Run trigger result:', result.data);
                } else {
                    showToast(result.message || 'รันไม่สำเร็จ', 'error');
                }
            } catch (error) {
        ...
        async function createBookingFlexMessage(bookingData) {
                                ...
                                contents: [
                                    {
                                        ...
                                        contents: [
                                            ...
                                            { type: 'text', text: 'ห้องประชุม:', size: 'sm', color: '#8c8c8c', flex: 2 },
                                            { type: 'text', text: roomName || 'ไม่ระบุห้อง', size: 'sm', color: '#06c755', weight: 'bold', flex: 4, wrap: true }
                                        ]
                                    },
                                    {
                                        type: 'box',
                                        ...
                                        contents: [
                                            { type: 'text', text: '👤', size: 'md', flex: 0 },
                                            { type: 'text', text: 'ผู้จอง:', size: 'sm', color: '#8c8c8c', flex: 2 },
                                            { type: 'text', text: userName || 'ไม่ระบุชื่อ', size: 'sm', weight: 'bold', flex: 4, wrap: true }
                                        ]
                                    },
                                    {
                                        type: 'box',
                                        ...
                                        contents: [
                                            { type: 'text', text: '📅', size: 'md', flex: 0 },
                                            { type: 'text', text: 'วันที่:', size: 'sm', color: '#8c8c8c', flex: 2 },
                                            { type: 'text', text: formatDateShort(startTime) || 'ไม่ระบุวันที่', size: 'sm', flex: 4 }
                                        ]
                                    },
                                    {
                                        type: 'box',
                                        ...
                                        contents: [
                                            { type: 'text', text: '⏰', size: 'md', flex: 0 },
                                            { type: 'text', text: 'เวลา:', size: 'sm', color: '#8c8c8c', flex: 2 },
                                            { type: 'text', text: (formatTime(startTime) || 'ไม่ระบุ') + ' - ' + (formatTime(endTime) || 'ไม่ระบุ'), size: 'sm', flex: 4 }
                                        ]
                                    },
                                    {
                                        type: 'box',
                                        ...
                                        contents: [
                                            { type: 'text', text: '👥', size: 'md', flex: 0 },
                                            { type: 'text', text: 'ผู้เข้าร่วม:', size: 'sm', color: '#8c8c8c', flex: 2 },
                                            { type: 'text', text: (attendees || 0) + ' คน', size: 'sm', flex: 4 }
                                        ]
                                    },
                                    {
                                        type: 'box',
                                        ...
                                        contents: [
                                            { type: 'text', text: '🆔', size: 'md', flex: 0 },
                                            { type: 'text', text: 'รหัสการจอง:', size: 'sm', color: '#8c8c8c', flex: 2 },
                                            { type: 'text', text: bookingId?.substring(0, 8) + '...' || 'ไม่ระบุ', size: 'sm', flex: 4 }
                                        ]
                                    }
                                ]
                            },
            ...
            if (approvedByName) {
                flexMessage.contents.body.contents.push({
                    ...
                    cornerRadius: '8px',
                    contents: [
                        { type: 'text', text: '✅ อนุมัติโดย:', size: 'sm', weight: 'bold', color: '#2e7d32' },
                        { type: 'text', text: approvedByName, size: 'sm', color: '#1b5e20', weight: 'bold', wrap: true },
                        { type: 'text', text: approvedAt ? formatDateTime(approvedAt) : '', size: 'xs', color: '#4caf50', margin: 'xs' }
                    ]
                });
            }
            ...
            if (rejectedByName) {
                flexMessage.contents.body.contents.push({
                    ...
                    cornerRadius: '8px',
                    contents: [
                        { type: 'text', text: '❌ ปฏิเสธโดย:', size: 'sm', weight: 'bold', color: '#c62828' },
                        { type: 'text', text: rejectedByName, size: 'sm', color: '#b71c1c', weight: 'bold', wrap: true }
                    ]
                });
            }
            ...
            if (cancelledByName) {
                flexMessage.contents.body.contents.push({
                    ...
                    cornerRadius: '8px',
                    contents: [
                        { type: 'text', text: '❌ ยกเลิกโดย:', size: 'sm', weight: 'bold', color: '#c62828' },
                        { type: 'text', text: cancelledByName, size: 'sm', color: '#b71c1c', weight: 'bold', wrap: true }
                    ]
                });
            }
            ...
            if (reason) {
                flexMessage.contents.body.contents.push({
                    ...
                    cornerRadius: '8px',
                    contents: [
                        { type: 'text', text: '📝 เหตุผล:', size: 'sm', weight: 'bold', color: '#e65100' },
                        { type: 'text', text: reason, size: 'sm', color: '#bf360c', wrap: true }
                    ]
                });
            }
            ...
            if (description) {
                flexMessage.contents.body.contents.push({
                    ...
                    cornerRadius: '8px',
                    contents: [
                        { type: 'text', text: '📄 รายละเอียดเพิ่มเติม:', size: 'sm', weight: 'bold' },
                        { type: 'text', text: description, size: 'sm', color: '#424242', wrap: true }
                    ]
                });
            }
            ...
            if (createdAt) {
                flexMessage.contents.body.contents.push({
                    ...
                    margin: 'md',
                    contents: [
                        { type: 'text', text: '📌 จองเมื่อ: ' + formatDateTime(createdAt), size: 'xs', color: '#9e9e9e', align: 'center' }
                    ]
                });
            }
            ...
            if (bookingId && liff.isInClient()) {
                flexMessage.contents.footer.contents.push({
                    type: 'button',
                    action: { type: 'uri', label: '📅 ดูรายละเอียดในปฏิทิน', uri: bookingDetailUrl },
                    style: 'secondary',
                    margin: 'sm'
                });
            }
        ...
        window.bookOnSelectedDate = function () {
            if (!state.selectedDate) {
                showToast('กรุณาเลือกวันที่จากปฏิทิน', 'warning');
                return;
            }

            const dateStr = formatDateForInput(state.selectedDate);
            ...
            const startSelect = document.getElementById('booking-start');
            const endSelect = document.getElementById('booking-end');
            if (startSelect && [...startSelect.options].some(opt => opt.value === '09:00')) {
                startSelect.value = '09:00';
            }
            if (endSelect && [...endSelect.options].some(opt => opt.value === '10:00')) {
        ...
        window.selectDate = async function (dateStr) {
            ...
            if (result.success && result.data.bookings.length > 0) {
                ...
                $.selectedDateTitle.textContent = `📅 ${dateThai} (ว่าง)`;
                $.selectedDateBookingsList.innerHTML = `
                    <div class="text-center py-4 text-gray-400">ไม่มีตารางการจองในวันนี้</div>
        ...
        function loadManagerData() {
            // Operators retain booking/room operations, while admin-only data
            // (pending approvals, users, and settings) is never loaded.
            const requests = [
            ...
            Promise.all(requests).catch(console.warn);
        }
        ...
        window.approveBooking = async function (bookingId, btn, skipNotification = false) {
            ...
            if (result.success) {
                showToast(result.message || 'อนุมัติการจองสำเร็จ');
        ...
        window.adminCancelBooking = async function (bookingId, btn) {
            ...
            try {
                ...
                if (result.isConfirmed) {
                    ...
                    if (apiResult.success) {
                        showToast(apiResult.message || 'ยกเลิกการจองสำเร็จ (ส่งแจ้งเตือนไปยังผู้จองแล้ว)');
        ...
        window.toggleProjector = function() {
            ...
            if (projectorContainer) {
                if (useComp) {
                    projectorContainer.classList.remove('hidden');
                } else {
        ...
        window.toggleZoomLink = function() {
            ...
            if (format === 'online') {
                ...
                linkInput.required = false;
                linkInput.value = '';
                if (onsiteSection) onsiteSection.classList.remove('hidden');
        ...
        window.saveBooking = async function (e) {
            ...
            try {
                ...
                if (format === 'onsite') {
                    ...
                    if (useComp) extras.push('คอมพิวเตอร์');
                    if (useComp && useProj) extras.push('โปรเจคเตอร์');
                } else if (format === 'online') {
                    if (document.getElementById('booking-online-camera')?.checked) extras.push('กล้อง');
                    if (document.getElementById('booking-online-mic')?.checked) extras.push('ไมค์');
                    if (document.getElementById('booking-online-computer')?.checked) extras.push('คอมพิวเตอร์');
                }
                if (extras.length > 0) {
                    desc += `\n* ต้องการใช้: ${extras.join(', ')}`;
                }
        ...
        window.rejectBooking = async function (bookingId, btn) {
            ...
            try {
                ...
                if (isPastBooking) {
                    ...
                    if (apiResult.success) {
                        showToast(apiResult.message || 'ปฏิเสธการจองสำเร็จ (ไม่มีการแจ้งเตือน)');
                ...
                if (result.isConfirmed) {
                    ...
                    if (apiResult.success) {
                        showToast(apiResult.message || 'ปฏิเสธการจองสำเร็จ');
        ...
        function renderUsers() {
            ...
            $.usersList.innerHTML = filtered.map(u => `
                <div class="user-row">
                    ...
                    <span class="role-badge role-${u.role}">${{ admin: 'ผู้ดูแล', manager: 'ผู้ดำเนินการ', operator: 'ผู้ดำเนินการ', user: 'ผู้ใช้' }[u.role]}</span>
                    ${state.isAdmin ? `
        ...
        window.saveUserRole = async function (e) {
            ...
            if (result.success) {
                ...
                const roleNames = { admin: 'ผู้ดูแลระบบ', manager: 'ผู้ดำเนินการ', operator: 'ผู้ดำเนินการ', user: 'ผู้ใช้ทั่วไป' };
                await sendBookingToChat({
        ...
        window.saveProfile = async function (e) {
            ...
            const data = {
                ...
                lineUserId: state.user.lineUserId,
                // ส่งค่าปิด/เปิดการแจ้งเตือนไปที่ server ด้วย (ไม่ใช่เก็บแค่ localStorage)
                notificationsEnabled: $.profileNotificationsEnabled ? $.profileNotificationsEnabled.checked : true
            };
        ...
        window.switchTab = function (tab) {
            ...
            if (tab === 'admin' && state.isManager) {
                loadManagerData();
                if (state.isAdmin) checkTriggerStatus();
            }
        };
        ...
        document.querySelectorAll('.admin-tab-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                if (!state.isAdmin && ['pending', 'users', 'settings'].includes(btn.dataset.adminTab)) return;
                ...
                if (btn.dataset.adminTab === 'rooms') {
                    ...
                    loadPendingBookings();
                } else if (btn.dataset.adminTab === 'settings') {
                    checkTriggerStatus();
                }
            });
        });
